import { describe, it, expect, beforeAll, afterAll } from 'vitest';
import { pool, insertQuestion, getRandomQuestion, initDb } from '../db.js';

describe('Database Category Selection', () => {
    const testTopic = 'TEST_SPECIFIC_TOPIC_' + Date.now();
    const testQuestionId = 'test_q_topic_' + Date.now();

    beforeAll(async () => {
        // Ensure the database table is created before inserting
        await initDb();

        // Insert a question with a specific topic
        await insertQuestion({
            id: testQuestionId,
            question: 'Mock question for category testing',
            context: 'Mock context',
            referenceAnswer: 'Mock answer',
            citations: ['doc1']
        }, testTopic);
        
        // Insert another question with a different topic to ensure filtering works
        await insertQuestion({
            id: testQuestionId + '_other',
            question: 'Mock question another',
            context: 'Mock context',
            referenceAnswer: 'Mock answer',
            citations: ['doc2']
        }, 'OTHER_TOPIC_' + Date.now());
    });

    afterAll(async () => {
        // Cleanup test data
        await pool.query('DELETE FROM questions WHERE id IN ($1, $2)', [testQuestionId, testQuestionId + '_other']);
        // Close pool to let tests exit cleanly
        await pool.end();
    });

    it('should fetch a random question strictly filtered by the selected topic', async () => {
        const result = await getRandomQuestion(testTopic);
        
        expect(result).toBeDefined();
        expect(result?.id).toBe(testQuestionId);
        expect((result as any)?.topic).toBe(testTopic); // Ensures the topic matches via the 'LIKE' query
    });
    
    it('should return null when fetching a non-existent topic', async () => {
        const result = await getRandomQuestion('NON_EXISTENT_TOPIC_123456789');
        expect(result).toBeNull();
    });
});
