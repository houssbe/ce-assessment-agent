...
export class GCPKnowledgeService {
  ...
  private async runResearchPhase(
    ...
    if (options?.prefetchDocNames && options.prefetchDocNames.length > 0) {
      ...
      const uncachedNames = options.prefetchDocNames.filter(
        (n) => !this.docCache.has(n),
      );
      const cachedNames = options.prefetchDocNames.filter((n) =>
        this.docCache.has(n),
      );
      console.log(
      ...
      try {
        ...
        if (uncachedNames.length > 0) {
          const docResult = await this.callMCP("tools/call", {
            name: "batch_get_documents",
            arguments: { names: uncachedNames },
          });
          const docsArray: Array<{
            content?: string;
            name?: string;
            uri?: string;
          ...
          for (const doc of docsArray) {
            ...
            console.log(
              `[Pre-fetch] ✅ Fetched & cached: ${docName} (${content.length} chars)`,
            );
          }
        }
    ...
    if (!prefetchSucceeded && options?.prefetchQuery) {
      ...
      try {
        const searchResult = await this.callMCP("tools/call", {
          ...
          arguments: { query: options.prefetchQuery },
        });
        ...
        const snippetText = chunks
          ...
          .join("\n\n");
        console.log(
          `[Pre-fetch] ✅ Using ${chunks.length} search snippets (${snippetText.length} chars), ${parentNames.length} parent doc(s) for citations`,
        );

        const citationList =
          parentNames.length > 0 ? parentNames.join(", ") : "N/A";
        enrichedPrompt = `=== OFFICIAL DOCUMENTATION (fetched live from Developer Knowledge API) ===
...
IMPORTANT: Base your response exclusively on the documentation above. For the "citations" field, use these document resource names: ${citationList}`;
        ...
        console.warn(
          `[Pre-fetch] ⚠️ Pre-fetch failed, agent will rely on tools:`,
          err,
        );
      }
    }
    ...
    if (!prefetchSucceeded) {
      console.warn(
        `[Research] Pre-fetch failed or was skipped. Model will use training data.`,
      );
    }
  ...
  async evaluateAnswer(
    ...
    const research = await this.runResearchPhase(
      ...
      EVALUATION_SCHEMA,
      {
        ...
        prefetchQuery: hasCitations
          ? undefined
          : question.question.split(/\s+/).slice(0, 10).join(" "),
        onProgress,
      },
    );
